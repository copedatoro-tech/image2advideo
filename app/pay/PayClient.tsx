...
export default function Home() {
  ...
  // 🔥 PASUL 1: Trimitem imaginile + setările la /api/job/create
  const createJob = async () => {
    ...
    if (!data.jobId) {
      alert("Eroare la crearea jobului");
      return null;
    }
  ...
  // 🔥 PASUL 2: Trimitem jobId către Stripe
  const handleCreateVideo = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/stripe/checkout", {
        ...
        body: JSON.stringify({
          price: totalPrice,
          jobId, // 🔥 trimitem jobId către Stripe
        }),
      });
